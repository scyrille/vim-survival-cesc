...
X_pathways <- tcga$pathway$clin_dna_rna %>%
  dplyr::select(starts_with("genomic_pathway"),
                starts_with("hallmark_"))%>%
  colnames()

tcga_vimp <- tcga$pathway$clin_dna_rna %>%
  dplyr::select(time, event, all_of(X_pathways))

# ranger ------------------------------------------------------------------

vimp_ranger_fit <- ranger::ranger(Surv(time, event)~., tcga_vimp,
                                  importance = "permutation", 
                                  splitrule = "logrank",
                                  seed = 123)

saveRDS(vimp_ranger_fit, 
        here::here("outputs","results", "tcga_vimp_ranger_fit.rds"))

# survex ------------------------------------------------------------------

horizon_time <- 24 
times <- sort(unique(c(0, tcga_vimp$time, horizon_time)))
...
ranger_fit <- ranger::ranger(Surv(time, event)~., tcga_vimp,
                             importance = "none", 
                             splitrule = "logrank",
                             seed = 123)

# Model performance
ranger_exp <- survex::explain(ranger_fit, data = tcga_vimp,
                              y = Surv(tcga_vimp$time, tcga_vimp$event),
